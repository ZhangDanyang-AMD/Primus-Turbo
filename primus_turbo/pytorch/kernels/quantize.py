...
def convert_to_mxfp8(
    ...
    data_lp = torch.empty(data_shape, dtype=float8_dtype_pt,
                          device=data_hp.device).reshape(-1, data_shape[-1])
    
    if is_2d_block:
        scales_shape = (*data_shape[:-2], data_shape[-2] // block_size,
                        data_shape[-1] // block_size)
    else:
    ...
    return data_lp.reshape(data_shape).transpose(
